...
describe('Comprehensive Integration Tests', () => {
  ...
  describe('Attendance', () => {
    it('Employee can check in and out', async () => {
      ...
      if (inRes.status !== 200) console.log('CHECK-IN FAIL:', inRes.body);
  ...
  describe('Data Integrity (Part 21)', () => {
    it('should reject deactivation of team with active members', async () => {
      const admin = await createTestUser({ role: UserRole.ADMIN });
      const team = await createTestTeam('Integrity Team');
      const emp = await createTestUser({ role: UserRole.EMPLOYEE, team: team._id.toString() });

      const res = await authPut(`/teams/${team._id}`, admin.accessToken, { isActive: false });
      expect(res.status).toBe(400);
      expect(res.body.message).toContain('active employee');
    });

    it('should prevent overlapping leaves', async () => {
      const leaveType = await LeaveType.create({ name: 'Sick Leave', defaultAllocation: 12, isActive: true });
      const emp = await createTestUser({ role: UserRole.EMPLOYEE });
      await LeaveBalance.create({
        employee: emp.id, leaveType: leaveType._id, year: new Date().getFullYear(),
        allocation: 12, used: 0, available: 12,
      ...
      const res1 = await authPost('/leave/apply', emp.accessToken, {
        leaveTypeId: leaveType._id.toString(),
        startDate: startDate.toISOString(),
        endDate: endDate.toISOString(),
      ...
      const res2 = await authPost('/leave/apply', emp.accessToken, {
        leaveTypeId: leaveType._id.toString(),
        startDate: addDays(new Date(), 11).toISOString(),
        endDate: addDays(new Date(), 13).toISOString(),
        reason: 'Second request overlapping',
      });
      expect(res2.status).toBe(400);
      expect(res2.body.message).toContain('pending or approved');
    });
    
    it('should reject deleting a task with children', async () => {
      const admin = await createTestUser({ role: UserRole.ADMIN });
      const parentTask = await authPost('/tasks', admin.accessToken, {
        title: 'Parent Task', assignedTo: admin.id, priority: 'low'
      });
      const parentId = parentTask.body._id || parentTask.body.data?._id;
      
      await authPost('/tasks', admin.accessToken, {
        title: 'Child Task', assignedTo: admin.id, priority: 'low', parentTask: parentId
      });
      
      const res = await authDelete(`/tasks/${parentId}`, admin.accessToken);
      expect(res.status).toBe(400);
      expect(res.body.message).toContain('subtasks');
    });
  });

  describe('Notifications', () => {
    it('Task creation should generate a notification for assignee', async () => {
      const admin = await createTestUser({ role: UserRole.ADMIN });
      const emp = await createTestUser({ role: UserRole.EMPLOYEE });
      
      await authPost('/tasks', admin.accessToken, {
        title: 'Test Notification Task', assignedTo: emp.id, priority: 'high'
      });
      
      const res = await authGet('/notifications', emp.accessToken);
      expect(res.status).toBe(200);
      expect(res.body.data.notifications.length).toBeGreaterThan(0);
      expect(res.body.data.notifications[0].title).toBe('New Task Assigned');
    });
  });

  describe('Daily Progress & Weekly Reports', () => {
    it('Employee can submit daily progress', async () => {
      const emp = await createTestUser({ role: UserRole.EMPLOYEE });
      const task = await authPost('/tasks', emp.accessToken, {
        title: 'My Task', assignedTo: emp.id, priority: 'low'
      });
      // Employee assigning to self is blocked by previous rules, so let admin do it
      const admin = await createTestUser({ role: UserRole.ADMIN });
      const adminTask = await authPost('/tasks', admin.accessToken, {
        title: 'Admin Task', assignedTo: emp.id, priority: 'low'
      });
      const taskId = adminTask.body._id || adminTask.body.data?._id;
      
      const res = await authPost('/daily-progress', emp.accessToken, {
        date: new Date().toISOString(),
        tasksProgress: [{ task: taskId, progress: 50, notes: 'Halfway done' }]
      });
      expect(res.status).toBe(200);
    });

    it('Team Lead cannot submit duplicate weekly report', async () => {
      const team = await createTestTeam('Report Team');
      const teamLead = await createTestUser({ role: UserRole.TEAM_LEAD, team: team._id.toString() });
      const weekStartDate = new Date().toISOString();
      const weekEndDate = addDays(new Date(), 7).toISOString();

      const payload = {
        teamId: team._id.toString(),
        weekStartDate, weekEndDate, 
        metrics: { tasksCompleted: 1, tasksPending: 0, overdueTasks: 0, completionRate: 100 },
        achievements: 'Did great',
        completedWork: 'Stuff',
        incompleteWork: 'None',
        blockers: 'None',
        employeeContributions: 'Good',
        goals: 'Many',
        missedGoals: 'None',
        nextWeekPriorities: 'More',
        risks: 'None',
        notes: 'None'
      };

      const res1 = await authPost('/weekly-report', teamLead.accessToken, payload);
      expect(res1.status).toBe(201);

      const res2 = await authPost('/weekly-report', teamLead.accessToken, payload);
      expect(res2.status).toBe(400);
